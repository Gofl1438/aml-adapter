...
### 3. Тесты контроллера (`ClientCheckController`)

Эти тесты проверяют, что REST-контроллер возвращает правильные HTTP-ответы и JSON-сообщения для различных сценариев ошибок.  
Они используют `MockMvc` и не поднимают весь контекст приложения, что делает их быстрыми и изолированными.

#### Запуск тестов контроллера
[ClientCheckControllerTest.java](controller/ClientCheckControllerTest.java)

#### Что проверяют тесты

| Название теста | Что проверяет | Ожидаемый результат |
|----------------|---------------|----------------------|
| `shouldReturnRetryRequired` | Контроллер возвращает 502 с `retryAfter` при сервисной ошибке (`RetryRequired`) | HTTP 502, `{"error": "Результат не определен.", "retryAfter": 300}` |
*Запуск всех тестов

*Пкм по папке с тестами в IDE и "Run Tests in `org.example.amladapter`"