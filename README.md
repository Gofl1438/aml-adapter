...
Готового JAR у адаптера нет (в `target/` лежит только `.jar.original` — сборка падала на тестах),
поэтому его надо собрать. Тесты пропускаем: два интеграционных теста красные по историческим причинам.
